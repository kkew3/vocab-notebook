...
def test_insert_memos():
    ...
    m.upsert_word(conn, 'baz', 'Baz', 'B-a-z', [], 1)
    m.insert_memos(conn, [
        ('foo', datetime.datetime(2023, 1, 5), 5, '+'),
        ('bar', datetime.datetime(2023, 1, 5), 1, '-'),
        ('baz', datetime.datetime(2023, 1, 7), 1, 2),
        ('foo', datetime.datetime(2023, 1, 8), 5, '-'),
        ('baz', datetime.datetime(2023, 1, 8), 2, '.'),
    ])
    sorted(
        m.find_word_memo(conn, 'foo'), key=lambda x: x['date']) == [
            {
...
def test_update_word_familiarity():
    ...
    m.upsert_word(conn, 'bar', 'Bar', 'B-a-r', ['bar baz qux', 'lorem'])
    m.upsert_word(conn, 'baz', 'Baz', 'B-a-z', [], 1)
    m.update_word_familiarity(conn, 3, 'foo')
    assert sorted(
        m.find_all_words(conn), key=lambda x: x['word']) == [
            {
                'word': 'bar',
    ...
    m.update_word_familiarity(conn, 2, 'qux')
    assert sorted(
        m.find_all_words(conn), key=lambda x: x['word']) == [
            {
                'word': 'bar',
...
def test_update_word_familiarites():
    conn = sqlite3.connect(':memory:')
    m.init_db_if_not_exists(conn)
    m.upsert_word(conn, 'foo', 'Foo', None, ['foo bar baz'], 4)
    m.upsert_word(conn, 'bar', 'Bar', 'B-a-r', ['bar baz qux', 'lorem'])
    m.upsert_word(conn, 'baz', 'Baz', 'B-a-z', [], 1)
    m.update_word_familiarities(conn, [(3, 'foo'), (2, 'bar'), (5, 'qux')])
    assert sorted(
        m.find_all_words(conn), key=lambda x: x['word']) == [
            {
                'word': 'bar',
